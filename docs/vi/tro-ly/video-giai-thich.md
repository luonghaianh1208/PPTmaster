...
2. Kiểm cú pháp và giới hạn: `python tools\vi\video_ma.py projects\_video\<tên_video> --plan-only`. Ảnh quá 8 MB hay thiếu nguồn, tên biểu tượng sai là lỗi `canh`; xử lý theo mục "Ảnh thật" của `docs/vi/tro-ly/canh-video.md` hoặc chọn tên trong các tên mà lỗi gợi ý (tên biểu tượng là tiếng Anh, tra "Bảng tra biểu tượng").
